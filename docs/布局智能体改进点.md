...
在系统迭代早期，采用了 **“关系型数据库 + 向量数据库** 的分离架构。虽然解决了基本的存储与搜索问题，但在布局场景下遇到了核心挑战：

- **拓扑表达缺失**：关系型数据库难以高效表达元件之间的“左邻右舍”和“层级包含”等复杂物理拓扑。
- **数据一致性**：元数据与向量特征存储在两个系统中，入库与更新时的事务一致性难以保障。
- **查询链路长**：每次推荐需要跨库进行多次查询和特征对比，响应延迟高。

**本次改造全面迁移至 Neo4j 原生图数据库：**

- **统一存储**：将箱柜、面板、元件 BOM 以及高维特征向量全部整合进 Neo4j 中。
- **向量检索**：利用 Neo4j 原生的 向量索引，实现了“在特定图约束下进行向量搜索”的能力（例如：仅在指定柜体系列内搜索相似面板）。
- **知识挖掘**：通过图算法直接在数据库内沉淀元件共现关系，将原来深藏在向量空间的“黑盒特征”转化为直观的图谱关系。
...
基于图向量检索与知识图谱挖掘，实现了**多路召回 + 置信度融合**的智能 BOM 补全：

- **环境召回 (Environment Recall)**：基于箱体尺寸、系列、进线方式等非 BOM 特征，召回在类似物理约束下的历史方案。
- **BOM 召回 (BOM Recall)**：基于当前已有元件的分布特征，寻找相似的BOM方案。
- **图谱共现召回 (Graph Recall)**：利用 Neo4j 中的 `CO_OCCURS_WITH` 关系，挖掘历史数据中与当前元件高频“成对出现”的邻居部件。
...
- **质量衡量**：通过“采纳/推荐”的比值，系统可以实时感知每个历史模板在实际业务中的受欢迎程度和质量得分，为后续的推荐逻辑提供数据支撑。

![neo4j](images/schema_select.png)

### 3. 热门方案推荐

除了单纯的几何相似度，还引入了**热度排序维度**：

- **高频优先**：在召回结果相似度相近时，系统会优先推送 `adoption_count` 较高的“明星方案”。
- **进化学习**：随着用户的使用，图数据库中的统计数据会不断动态刷新，实现“用的越多、推的越准”的正向循环。
...
- **客户端交互**：将布局智能体封装为独立桌面exe，与 SuperBox 通过 IPC 进行数据交互。
- **一键打包**：通过 `build_electron.bat` 一键打包，方便快速迭代。

### 2. AI 方案推荐与迁移

- **RAG 语义检索**：将 SuperBox 的 BOM 清单与柜体属性作为 Query，在图数据库中进行向量检索，返回最匹配的布局参考。
- **布局平移优化**：应用方案时，并非简单的坐标复制，而是基于 **OR-Tools CP-SAT 求解器**。以历史方案为“约束骨架”，在满足当前 SuperBox 实际柜体尺寸约束下，自动计算不重叠、边距最优的新坐标，实现布局的智能迁移。

### 3. 布局方案上传

- **经验沉淀**：用户在 SuperBox 中微调完成的布局，可一键上传到布局方案库。
- **自动特征化**：后端自动提取该方案的几何特征、拓扑关系并重新计算向量指纹，将其作为新的“经验节点”沉淀到 Neo4j 知识图谱中，完成知识的自动化积累。
