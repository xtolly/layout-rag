...
class EditCabinetInput(BaseModel):
    cabinet_id: str = Field(description="要修改的柜体 ID（定位用，通过 get_current_selection 或 get_scheme_summary 获取）")
...
class EditPanelInput(BaseModel):
    panel_id: str = Field(description="要修改的面板 ID（定位用，通过 get_current_selection 或 get_scheme_summary 获取）")
...
class EditPartInput(BaseModel):
    part_id: str = Field(description="要修改的元件 ID（定位用，通过 get_current_selection 或 get_scheme_summary 获取）")
...
@tool
async def get_scheme_summary() -> str:
    """
...
4. 用户提到“当前面板”或“当前柜体”时，先调用 get_current_selection；用户通过名称描述目标时，先调用 get_scheme_summary 找到对应 ID。
5. 每次工具调用后，简要说明执行了什么。
...
# ──────────────────────────────────────────────────────────────
TOOLS = [add_cabinets, add_panels, add_parts, edit_cabinet, edit_panel, edit_part, delete_cabinet, delete_panel, delete_part, get_current_selection, get_scheme_summary]
