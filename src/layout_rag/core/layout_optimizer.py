...
class LayoutOptimizer:
    ...
    def apply_layout_template(
        ...
        Args:
            template_data:      主参考模板（含 schema.parts / arrange）。
            project_data:       当前项目数据（含 schema.parts / schema.panel_size），
        ...
        curr_parts = project_data.get("schema", {}).get("parts", [])
        curr_size  = project_data.get("schema", {}).get("panel_size", default_panel)
        tpl_parts  = template_data.get("schema", {}).get("parts", [])
        tpl_arrange = template_data.get("arrange", {})
        tpl_size   = template_data.get("schema", {}).get("panel_size", default_panel)
    ...
    def _build_fallback_type_index(
        ...
        for tpl in fallback_templates:
            tpl_parts  = tpl.get("schema", {}).get("parts", [])
            tpl_arrange = tpl.get("arrange", {})
            tpl_size   = tpl.get("schema", {}).get("panel_size", default_panel)
